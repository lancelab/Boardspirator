...
(function( $ ){var tp  =  $.fn.tp$;	
	...
	$.fn.tp$[self_name]=(function()
	{
		...
		self.create_select_el=function(argr_,argt_){
			var argr={
			...
			core.rpaste(argr,argr_);
			core.tpaste(argt,argt_);
			...
			var reset_styles=function(){
				...
				var ibox_height=argr.options.length*hitem;
				var ibox_wrapper_height=hbox-bottom_indent;
				if(ibox_height<ibox_wrapper_height){
					ibox_wrapper_height=ibox_height;
			...
			var bind_navigation_events=function(){
				...
				$(sel['top_strip']).bind('click',function(event){
					if(argr.options.length>1){
						...
						argr.callback(0,argr.options[0]);
					}
					return true;
				});
				...
				$(s_d).bind('click',function(event){
					var pos=parseInt(itembox.style.top);
					pos -= hitem;
					if(pos+(argr.options.length+1)*hitem>hbox) itembox.style.top=pos+'px';
					return false;
				});
			...
			select_el.reset=function(argr_,argt_){
				core.rpaste(argr,argr_);
				core.tpaste(argt,argt_);
			...
			//TODm better:
			//select_el.reset({},{dont_reset_styles:true});
