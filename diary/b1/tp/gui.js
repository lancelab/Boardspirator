...
(function( $ ){var tp  =  $.fn.tp$;	
	...
	$.fn.tp$[self_name]=(function(){
		...
		//==================================
		// Create triangle in HTML 4.01
		//==================================
		self.create_triangle=function(arg_){
			var arg={
				direction: 'down',
				style		:{
					//don't put pos ... 'absolute', it's hard coded later:
					width			: 20,
					height			: 20,
					//top			: 0,//legal
					//left			: 0,//legal
			...
			//c onsole.log('normalized style=',nstyle);
			...
			//tp$.debug(triangle.style.position,'generated triangle.style.position');
			arg.parent.appendChild(triangle);
			return {div:triangle, arg:arg};
		};//self.make_triangle


		//Adds 'px' to dimensional properties of a style
		...
		self.cornerize=function(arg,el){
			...
			//TODm Exclude from this feature:
			...
			if(addition){
				cssText=$.trim(cssText);
					...
					// Apparently:	AppleWebkit breaks zIndex:
					//				converts 3e06 to nothing
					//				Doing fix for this:
					//if( style.zIndex || style.zIndex === 0 ){
					//	var stashedZIndex = style.zIndex;
					//}
					//tp.debstorage = "\nzBefore adding rounds: zind="+el.style.zIndex;
					//tp.debstorage += "\ncssText"+cssText;
					//tp.debstorage += "\naddition"+addition;

				//Opera breaks on this ... fixing:
				if(cssText.charAt(cssText.length-1) !== ';'){
					cssText += ';';
				}
				//tp$.deb(style.cssText,'style.cssText');
				style.cssText=cssText+addition;

					// For case of AppleWebkit breaking zIndex:
					//tp.debstorage += "\nFinal style.cssText="+cssText+addition;
					//tp.debstorage += "\nAfter adding rounds: zind ="+el.style.zIndex;
