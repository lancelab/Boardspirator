	...
	gio.gui.init.controls_and_game_list =	function(){
		...
		//////////////////////////////////
			...
			*/
								...
								switch(option.title){
									...
									case 'Save Session' :
											var w = gio.data_io.core.save.object(
												gio.modes.sta_tic.db+'/game_sessions',
												gio.data_io.session.save());
									...
									case 'Load Session' :
											...
											var success = gio.data_io.session.load(w.session);
											if(!success){
												gio.cons_add('Problems loading session');
												return;
											}
											success = gio.navig.select_game_and_collection();
											if(!success){
												gio.cons_add('Problems loading session');
												return;
											}
			...
			/////////////// Next z-level. Slot 7 //////////////////////
			gio.controls.browse_solver=makeControl({
				name : 'solver',
				options :	[{	title:'Browse Solver',
								tooltip:'Toggles between play and solver modes'
							}],
				//redraw_scene : true,
				callback :	function(dummy, dummy_options, select_el){
								// This is a button event. It does not use "options" which is dummy_options.
								var mode = gio.solver.set_browser_mode(!gio.solver.browser_mode);
								// Do empty move:
								if( mode ) gio.getgs().gm.solver.browser.do_move();
								select_el.display.title = mode ? 'Go to Play' : 'Browse Solver';
								select_el.display.innerHTML = mode ? 'Go to Play' : 'Browse Solver';
							}
			});

			gio.gui.solver_select_el = makeControl({
					name : 'solver_stopper',
					options :	[{	title:'Stop Solver',
									tooltip:'stops solver running on current map'
								}],
					callback :	function(dummy, dummy_options, select_el){
									var gs = gio.getgs();
									var gm = gs.gm;
									var processor = gm.solver;
									var disp = select_el.display;
									var title = '"'+gm.parsed.final_title+'"';
									processor.stopped = !processor.stopped;
									
									if(!processor.stopped){
										// If needed, will autoresume this: gm.solver.do_searches();
										gm.solver.fire_up(gs.round.pos,'stop_when_solution_found');
									}
									gio.draw_status();
								}
			});
			...
			var help_callback=function(i,option){
				switch(option.title){
					...
					case 'Rules' :	
									...
									break;
					case 'About' :	gio.gui.procs.toggle_about_pane();
									break;
					case 'About Map' :
		...
		//------------------------
			...
			gio.domwrap.popups.help_div=gio.common_popup.popup_el;

			//Append about:
			gio.common_popup.update_owner(gstyle.popups.about);
			gio.domwrap.popups.about_div=gio.common_popup.popup_el;

			//Append map comments:
			w=$.extend({},gstyle.popups.about);
			w.owner='map_comments';
			gio.common_popup.update_owner(w);
			gio.domwrap.popups.map_comments_div=gio.common_popup.popup_el;
		//------------------------
		// ? Finalize common popup
		//========================
	};
