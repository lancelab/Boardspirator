	...
	rman.pos2map_script = function ( round, do_comap ) {
		...
		for( var yy = 0; yy < y_size; yy++ ) {
			...
			for( var xx = 0; xx < x_size; xx++ ) {
				...
				{
					//	Changes order to place dynamic units to the bottom for co-maps.
					var zz = do_comap ? (top - zz_virt) : zz_virt; 

					var lid = tower[ zz ];
					var uid = lid2uid[ lid ];
					var unit = units[ uid ];
					//.	Converts dynamic units to "targets" for co-maps.
					var symbol =	do_comap ? 
									cotable[ unit.cname ] :
									table[ unit.cname ];
				...
				if( short_symbols )
				{
					...
					// Cuts redundant periods if there are periods in the middle.
