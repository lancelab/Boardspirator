			...
			tp.core.each(game.interact[actor_colony_nam], function(peer_nam,val)
			{
				...
				//----------------------------------------------
				{
					...
					tp.core.each(punits, function(peer_ix,val)
					{
						...
						{
							msg=actor_colony_nam+' '+unit_ix+ ' is blocked by '+peer_nam+' '+peer_ix+'. Block is inside pos '+xnew+','+ynew+'...';
							return false;
						}
					});
					if(msg)return false;
				...
				//--------------------------------------------------
					...
					tp.core.each(punits, function(peer_ix,val)
					{
						...
						if(val.x === xnew && val.y === ynew) //blocker is on the way
						{
							...
							msg=actor_colony_nam+' '+unit_ix+' is blocked by ' + peer_nam+ ' ' + peer_ix + '. Deepness=' + deepness + '. Inside pos '+ xnew+','+ynew+'...';
