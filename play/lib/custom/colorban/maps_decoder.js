...
gio.colorban_maps_decoder=(function(){
	...
	// Names on right side of these tables
	// define possible collection(breed) names:
	//   for example: 		' ':'ground',
	//						'#':'wall_x',
	//						....		
	//=========================================
	...
	var finish_the_map=function(
		...
		//- - - - - - - - - - - - - - - - - - - - 
		var playpaths=[];
		var current_playpath=null;
		for(var y=cm.start; y<=cm.end; y++){
			var com=line=flines[y].replace(/\r/g,''); //vital for match

			pkey=extractKey(com)
			if(pkey[1]===':::' && pkey[2]==='playpath' && pkey[3]==='begin'){
				//explicit playpath beginning:			
				current_playpath=
			...
			if(w){
				...
				//to link a playpath:
				//for self-attaching maps and solutions:
				if(current_playpath){
					//c onsole.log('parsing pp. pkey=',pkey);
					if(pkey[1]===':::'){
						if(	pkey[2]==='game_key' ||
							pkey[2]==='collection_key' ||
							pkey[2]==='map_key'){
								current_playpath[pkey[2]]=pkey[3];
								continue;
						}
					}
				}
				current_playpath=null;
			}	
		//- - - - - - - - - - - - - - - - - - - - 
		//collect palypaths
		//----------------------------------------
