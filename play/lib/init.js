	...
	gio.compile_colony = function(colony_ix,colony)
	{
			...
			$.each(tiles_map, function(y,row){
				$.each(row, function(x, tile_from_map){
					...
					punit.x=x;
					punit.y=y;
					var unit_ix=punit.ix=count; //TODm dry?
					...
					$(t).bind('click',function(event){
						return gio.handle_click_on_unit(colony_ix, unit_ix);
					});
					switch(game.activity_role[nam]){
						case 'active':
							$(t).bind('dblclick',function(event){
								return gio.select_active_unit(colony_ix, unit_ix);
							});
							break;
					}
					//=========================================
			...
			//TODOm stub: for performance case: implement:
			//var inv=pos.inverse=[];
			//tp.core.each(pos,function(ix,punits){
			//});			
	};
